...
status: in progress; U0/U1/U1a implemented; U2b has bounded reads/proposals/transactions, compendium runs, identity/conflict, review/repair, dispatch-correlation, and initial stress fixtures; storage/reconstruction and external subagent-facility mapping remain; U3 blocked on U2b
---
...
- **Initial stress fixtures:**
  ...
  - ten distinct sequential work runs retained in one compendium projection;
  - 20 asynchronously prepared proposals committed in reverse completion order through one fenced transaction queue; a malformed successor cannot replace the accepted revision.
...
**Storage/reconstruction is complete for bounded transactions.** Transaction receipts now retain only operation metadata and content-addressed artifact references under `stages/merge/artifacts/`; they no longer embed a complete semantic stage. Candidate-disposition deltas are retained, so replay preserves accounting. Every 16 transaction revisions emits an immutable checkpoint; historical review binding reconstructs a requested revision from its nearest checkpoint plus validated delta receipts, checking parent linkage, artifact content hashes, and the reconstructed semantic content hash. The 20-transaction pressure fixture now proves content-addressed receipts, checkpoint creation, and review-time reconstruction of revision 1 after revision 20.

The final U2b stress additions are also complete: an unrelated global revision rebases only while a transaction's declared artifact read set still matches; a changed dependency is rejected; a cancelled extractor has a recorded terminal receipt and a scoped completed retry; and an edition-repeat compendium run submits an explicit identity `match` for its canonical Ada while retaining provenance from both editions. The latter exposed and fixed compendium provenance validation: shared source projection now occurs before a compendium batch validates an artifact that retains cross-run citations. `npm run build`, focused mem-import tests (23/23), and full `npm test` (121/121) passed.
...
- Add a model-backed adapter-discovery fixture using a deliberately different extension setup from the normal local test workspace. It must inspect the live tool catalog and map only sufficient installed facilities to the generic coordinator/worker contract (exact worker allowlist, child task/session identity, completion/failure, interruption/resume where available, and durable dispatch correlation). It must fail closed when that setup cannot supply the contract; never infer compatibility from an extension name alone.
