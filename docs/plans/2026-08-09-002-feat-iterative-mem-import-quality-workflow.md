...
>
> **Immediate TODO from tiny Alice validation:** Fix the repair-phase campaign handoff/discovery defect exposed by run `mir-e1427ba914a36b08335ddb46`: the parent persisted campaign `campaign-2f54292bfc027bbc2b59f3cd`, but the first fresh repair coordinator could not resolve it because its launch envelope omitted the campaign identifier. Make repair campaign identity discoverable through typed durable state or require/pass it explicitly in the repair handoff, and add a regression test proving a fresh repair coordinator can resolve a persisted campaign without guessing or filesystem inspection. Preserve the recovery evidence as a failure/recovery case, not as a reason to replay completed phases.
>
> **Resolution (2026-08-10):** The repair coordinator now reads `mem_import_quality_state` first, derives the persisted `campaignId`, and then reads the frozen campaign through typed tools. Profile fixtures and workflow guidance carry this handoff, and a fresh-service regression covers policy persistence, discovery, and assignment binding without filesystem inspection or another import.
