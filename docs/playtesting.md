...
For live playtests that need monitoring or interaction, use the active agent harness's supervised execution mechanism when available rather than an unattended inline shell run. Keep the session visible and interruptible so the agent or human can send input, read output, and stop the process cleanly.
