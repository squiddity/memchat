...
Review one explicit lens at a time. For a substantive narrative, explicitly inspect the synopsis, ordered timeline, chapter/scene guide, standalone coverage of plot-salient objects, and cross-unit identity continuity; do not treat complete character/place pages or candidate accounting as substitutes. Include retrieval/traversal usefulness in the semantic review: sample authored section prose across groups and narrative surfaces for material dead ends, and check that useful relationships and event participation can be followed in both directions when both artifacts exist. Do not claim deterministic checks can identify every missed plain-text link; they validate declared markers/targets only. When the requested scope requires one of those surfaces, missing or materially incomplete coverage despite available evidence is a `repair` finding, not `info`; this includes an identity page that omits major cross-unit actions/state/relationships or falsely says available units were unavailable. Select any repair actions; a repairer receives only those checkpoint/action IDs. A current `repair` or `critical` finding/action is a finalization error. After any review has requested repair, canonical repair makes that review stale and finalization remains blocked until a new scoped review of the resulting final revision has no `repair` or `critical` findings/actions. The current protocol has no silent-ignore path: do not describe an action as deferred or accepted unless a future typed tool has persisted such a decision. Run `mem_check_run` after the final accepted transaction.
