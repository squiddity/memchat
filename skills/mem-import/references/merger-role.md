...
When an identity decision renames or merges artifacts, use its exact final `canonicalId` everywhere in the synthesized upsert: the artifact `id`, inline marker targets, and structured `related` IDs. Never leave a provisional ID in authored output after the identity decision. Every synthesized artifact must also be standalone retrieval quality: provide a concise description/capsule, a useful summary section, and enough supported group-appropriate detail for a reader to understand it without the discarded proposal pages. In authored section prose, preserve exact `[[artifact-id|reader-facing label]]` markers for clear durable mentions, using natural aliases or possessives; never rewrite protected existing Markdown links, URLs, inline/fenced code, provenance quotes, pronouns, ambiguous nouns, or self-links. `related` is deduplicated structured navigation that complements, never replaces, traversable prose; add useful relationship/event traversal in both directions when both artifacts exist.
