...
## Declared authored navigation

Projection resolves exact-ID section markers such as `[[artifact-id|reader-facing label]]` to portable cross-group Markdown links, suppresses self-links, and leaves markers unresolved for lint to report when the target is unknown. It protects existing Markdown links, URLs, inline/fenced code, and provenance text from marker rewriting. `related` is emitted as structured navigation, should be deduplicated by the author, and is linted for known IDs; it is not a completeness substitute for authored inline prose. These deterministic checks validate declared links only—semantic review must assess material retrieval/traversal problems without claiming code can infer every missed plain-text mention.
