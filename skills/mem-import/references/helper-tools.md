...
- `mem_merge_inventory` / `mem_import_merge_inventory` return cursor-paginated canonical artifact summaries and exact revision/hash controls, capped at 100 entries. `mem_merge_read_artifact` reads one named artifact. Use these for substantive corpora.
- `mem_merge_read` / `mem_import_merge_state` retain complete snapshots only as legacy diagnostic/comparison surfaces; do not use them for substantive corpus work.
...
- `mem_merge_apply_batch({ ..., batch })` is the normal merger mutation surface: it accepts one to twelve proposal-backed upsert/delete operations plus an exact per-artifact read set, a current fence, and expected revision/hash. If only unrelated canonical content changed, a valid read set permits a bounded rebase; any changed/missing declared artifact fails stale. The service persists a delta receipt under `stages/merge/transactions/`, and earlier accepted batches survive later work failures.
