...
Launch a subagent with the assignment bootstrap and exactly `assignment.tools`. Production assignments bind exactly one immutable `planHash` and `clusterId`; the service derives the cross-unit `units` and qualified `unitId:candidateId` scope from that artifact. Do not broaden, regroup, or transcribe a different scope. Direct-scope fixture assignments may carry explicit unit/candidate scope.

## Steps

1. For a planned assignment, use its exact `units` and qualified `candidateIds` to call `mem_extraction_read_worker` directly for only those candidates; do not call extraction inventory first. Inventory is only for a direct-scope fixture assignment that genuinely lacks exact candidate scope. Treat candidate title, payload, metadata, and provenance together as sufficient primary evidence when they support the artifact claim without an explicit gap or contradiction. A recurring entity cluster may span units; synthesize it as one model-owned identity-aware shard.
...
5. Keep artifacts and provenance concise but semantically complete. For standalone retrieval, provide a concise description/capsule and useful summary, then add richer supported sections with progressive disclosure and group-appropriate details; preserve explicit provenance and never invent unsupported detail. Use narrow supporting ranges and avoid repeating the same source prose across artifact sections.

   In every authored section body—people, places, things, facts, style, and narrative surfaces—mark each clear durable mention of another artifact with the exact `[[artifact-id|reader-facing label]]` marker. Use natural aliases and possessives in labels. Do not link pronouns, ambiguous common nouns, the current artifact, existing Markdown links, URLs, inline/fenced code, or provenance quotes. Keep `related` structured and deduplicated, never as a replacement for traversable inline prose; when both artifacts exist, make useful relationships and event participation traversable in both directions without forced reciprocal noise.
