...
When running inside herdr/pi, use the same dedicated lower-pane pattern for any world-import or helper command that should be watched, may take more than a moment, or emits useful streaming output. This includes `world-import-run`, import test runs, lint/eval, provenance audits, repair loops, repeated source-search commands, and patch/re-emit loops. Keep inline execution for quick one-shot inspections only; do not hide supervised runs behind `nohup` or detached background shells unless no pane tool is available.
