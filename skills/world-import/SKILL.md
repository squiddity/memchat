...
When running inside herdr/pi, check that the `herdr` CLI is available and use it for supervised runs instead of defaulting to inline shell execution. Preferred flow: `herdr pane current`, `herdr pane split --current --direction down --cwd /home/squiddity/projects/memchat`, `herdr pane run <new-pane-id> '<command>'`, then supervise with `herdr pane read <pane-id>` or `herdr wait output <pane-id> --match <text>`. Use `herdr pane send-text` plus `herdr pane send-keys <pane-id> Enter` only when interacting with an already-running shell. Apply this to any world-import or helper command that should be watched, may take more than a moment, or emits useful streaming output, including `world-import-run`, import test runs, lint/eval, provenance audits, repair loops, repeated source-search commands, and patch/re-emit loops. Keep inline execution for quick one-shot inspections only; do not hide supervised runs behind `nohup` or detached background shells unless no pane tool is available.
