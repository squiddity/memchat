...
- For project code execution from herdr/pi, run project tool commands in a dedicated lower pane whenever they are meant to be watched, can take more than a moment, or produce meaningful streaming output. This includes `npm run ...` tool invocations such as imports, builds, tests, playtests, lint/eval runs, provenance audits, helper loops, repair loops, and source-search workflows. Avoid side panes for routine supervision. Do not hide these jobs behind `nohup`, background `bash`, or detached transcripts unless no pane tool is available; inline execution is only for quick one-shot inspection commands.
